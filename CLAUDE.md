...
Wrong: a 5-column kanban in `variant="split"` (the rail costs two board columns and the aside outgrows the board). Right: `variant="wide"` + `<StatStrip>` above the board, the work lists as the band below. The page header (greeting h1 + context line + primary action button) stays ABOVE the grid. Hand-rolling the page layout costs a `// layout-opt-out: <reason>` comment and is reserved for genuinely different page shapes (full-screen map, gallery-first) — the gate enforces this.
